                ...
                {destinations?.map((destination, index) => {
                    return (
                        <View key={index} className='mr-4 relative'>
                            <Image className='w-40 h-52 rounded-2xl' source={{ uri: destination.image }} />
                            <View className='absolute bottom-0 left-0 right-0 h-14 rounded-b-2xl items-center justify-center'>
                                <Text className='text-white font-bold text-xl'>{destination.name}</Text>
                            </View>
                        </View>
                    )
                })}
